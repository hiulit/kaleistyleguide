		...
		render: function () {
			...
			require(['text!'+ styleUrl], function (stylesheet) {
				...
				switch (result[2]) {
					...
					case 'stylus':
						require(['libs/stylus/stylus'], function() {
                            ...
                            if (config.mainPreprocessorStyleSheet) {
                                $.ajax({
                                    url: config.styleguideFolder + "/" + config.mainPreprocessorStyleSheet + "." + styleExt,
                                    async: false,
                                    cache: true,
                                    success: function(data){
                                        configPath = config.styleguideFolder + "/" + config.mainPreprocessorStyleSheet + "." + styleExt;
					...
					break;
					case 'less':
						require(['libs/less/less.min'], function(less) {
